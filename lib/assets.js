...
Assets.prototype.serveAsset = function (req, res, next) {
  ...
  var asset, mtime, buffer, length, bufferPath, contentType, resource;
  ...
  try {
    if (!this.options.compile) {
      ...
      if (this.options.flush) {
        resource = path.substr(path.lastIndexOf("/") + 1);
        ...
        bufferPath = this.options.buildDir + "/" + resource;
      } else {
  ...
  if (bufferPath) return fs.createReadStream(bufferPath).pipe(res);
